...
Readonly my $SCRIPT_DIR  => '/lxc/repo/adduser/deluser.d';
...
if ($?) {
	die "Cannot stop container '$user_name': $!\n";
}

# Remove container
system("$LXC_BIN remove $user_name");
if ($?) {
	die "Cannot remove container '$user_name': $!\n";
}

# Get deluser.d scripts
